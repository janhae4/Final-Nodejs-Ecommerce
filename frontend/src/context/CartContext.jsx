...
export const CartProvider = ({ children }) => {
  ...
  const addItemToCart = (product, variant, quantity = 1) => {
    setCartItems((prevItems) => {
      ...
      if (existingItemIndex > -1) {
        ...
        if (stockLimit !== undefined && newQuantity > stockLimit) {
          messageApi.warning(
            `Cannot add more. Only ${stockLimit} of ${product.name} (${variant.name}) in stock.`
          );
        ...
        if (stockLimit !== undefined && quantity > stockLimit) {
          messageApi.warning(
            `Cannot add ${quantity}. Only ${stockLimit} of ${product.name} (${variant.name}) in stock.`
          );
          return [
            ...prevItems,
    ...
    messageApi.success(
      `${product.name} ${variant ? `(${variant.name})` : ""} added to cart!`
    );
  };
