...
export const CartProvider = ({ children }) => {
  ...
  const [shippingCost, setShippingCost] = useState(5.0); // Example fixed shipping
  ...
  const addItemToCart = (product, variant, quantity) => {
    setCartItems((prevItems) => {
      const itemKey = product._id + (variant ? `-${variant._id}` : "");
      const existingItemIndex = prevItems.findIndex(
        (item) =>
          item.product._id + (item.variant ? `-${item.variant._id}` : "") ===
          itemKey
      );
      ...
      if (existingItemIndex > -1) {
        ...
        const stockLimit = variant?.inventory;
        if (stockLimit !== undefined && newQuantity > stockLimit) {
          messageApi.warning(
            `Cannot add more. Only ${stockLimit} of ${product.nameProduct} (${variant.name}) in stock.`
          );
        ...
        if (stockLimit !== undefined && quantity > stockLimit) {
          messageApi.warning(
            `Cannot add ${quantity}. Only ${stockLimit} of ${product.nameProduct} (${variant.name}) in stock.`
          );
          return [
            ...prevItems,
            { product, variant, quantity: stockLimit, key: itemKey },
          ];
        }
    ...
    messageApi.success(
      `${product.nameProduct} ${variant ? `(${variant.name})` : ""} added to cart!`
    );
