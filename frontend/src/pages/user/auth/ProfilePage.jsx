...
const ProfilePage = () => {
  ...
  const fetchAddresses = async () => {
    ...
    try {
      const res = await axios.get(
        "http://localhost:3000/api/users/shipping-addresses",
        {
          headers: { Authorization: `Bearer ${token}` },
          withCredentials: true,
        }
  ...
  const handleDeleteAddress = async (addressId) => {
    try {
      const token = localStorage.getItem("authToken");
      await axios.delete(
        `http://localhost:3000/api/users/shipping-addresses/${addressId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
          withCredentials: true,
        }
  ...
  const handleAddressFormSubmit = async (values) => {
    ...
    try {
      ...
      if (editingAddress) {
        const response = await axios.put(
          `http://localhost:3000/api/users/shipping-addresses/${editingAddress.id}`,
          values,
          {
            headers: { Authorization: `Bearer ${token}` },
            withCredentials: true,
  ...
  if (!userInfo) {
    return (
      <Layout>
        <div className="text-center p-8">Failed to load user data.</div>
      </Layout>
    );
  }
