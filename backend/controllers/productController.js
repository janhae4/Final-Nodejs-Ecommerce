...
exports.addComment = async (req, res) => {
    try {
        ...
        if (!user || !content) {
            return res.status(400).json({ status: false, message: 'User and content are required' });
        }

        const updatedProduct = await ProductService.addComment(productId, { user, content });
        ...
        res.status(500).json({ status: false, message: 'Error adding comment', error: err.message });
    }
};
